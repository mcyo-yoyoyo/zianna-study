...
export default function ErrorBook() {
  ...
  const intro =
    isChild
      ...
      : "在对应学科下拍卷识题。听写类错词由孩子在「听写」中核对，自动归入「语文·听写」记录，不与此处大题列表混行。";
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-0.5 text-xs text-slate-400">
          角标数字：孩子端「语文」为语文大题条数 + 听写核对次数；其他学科为本科大题条数。家长端角标为各科拍卷条数。
        </p>
      </div>
    ...
    <div className="space-y-8" key={subject}>
      ...
      {subject === "数学" && (
        <p className="rounded-xl border border-emerald-100/80 bg-emerald-50/50 px-3 py-2 text-sm text-slate-700">
          数学由家长在本科拍卷识题；此处仅对「数学」大题做 AI 举一反三闯关。
        </p>
      )}
      <div>
        <h3 className="text-sm font-bold uppercase tracking-wide text-slate-500">
        ...
        <p className="mb-3 text-sm text-slate-600">
          {subject === "语文" ? (
            <>与上面「听写次」不同：由家长本机为语文拍卷录入的大题，易错时在此闯关。</>
          ) : (
